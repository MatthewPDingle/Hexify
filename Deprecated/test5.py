...
def create_concentric_hex_patterns(center_x, center_y, radius, outer_black):
    ...
    black_white_colors = [outer_black, (255, 255, 255)][::2]

    # Function to draw 12-segmented pattern
    def draw_12_segment_pattern(base_radius, even_color, odd_color):
        num_segments = 12
        ...
        for i in range(num_segments):
            ...
            mask = np.zeros((8 * base_radius, 8 * base_radius), dtype=np.uint8)
    ...
    for i, size in enumerate(sizes[::-1]):
        if i % 2 == 0:
            ...
            print(black_white_colors)
            color = black_white_colors[i // 2 % 2]
            pattern = cv2.fillPoly(pattern, [RegularPolygon((radius, radius), numVertices=6, radius=size, orientation=np.pi / 2).get_verts().astype(int)], color)
        else:
            ...
            draw_12_segment_pattern(size, even_color, odd_color)
...
for center_x, center_y in hex_centers:
    if 0 <= center_x - hex_radius < output_image.shape[1] and 0 <= center_y - hex_radius < output_image.shape[0]:
        ...
        pattern_colors = [{'edge': palette[i], 'vertex': palette[(i + 1) % len(palette)]} for i in color_indices]
        
        # Determine black or white for the outermost layer
        outer_black = (0, 0, 0) if np.random.random() < 0.5 else (255, 255, 255)

        # Create concentric hex pattern using the selected colors
