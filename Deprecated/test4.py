...
# Function to create concentric hexagons with vertex and edge colors
def create_hex_pattern(center_x, center_y, radius, colors):
    ...
    for i, color in enumerate(colors):
        ...
        # Fill the whole inner hexagon with the edge color
        edge_color = color['edge']
        pattern = cv2.fillPoly(pattern, [inner_coords], edge_color)

        # Draw vertices
        for vertex in inner_coords:
            cv2.circle(pattern, tuple(vertex), radius=3, color=color['vertex'], thickness=-1)
