...
def closest_palette_color(avg_lab, palette_lab):
...
def create_hex_pattern(center_x, center_y, radius, avg_lab, palette_lab):
    ...
    for i in range(7, 0, -1):  # Iterate from outer to inner layer
        if i == 7:  # Outer layer (layer 7)
            ...
            layer_7_color = (0, 0, 0) if avg_lab[0] < 50 else (255, 255, 255)
            pattern = cv2.fillPoly(pattern, [inner_coords], tuple(map(int, layer_7_color)))
            ...
            color_2 = select_color_2(avg_lab, palette_lab, color_1, even_area, odd_area, layer_7_color, layer_areas[0] - layer_6_area)
...
for center_x, center_y in hex_centers:
    if 0 <= center_x - hex_radius < output_image.shape[1] and 0 <= center_y - hex_radius < output_image.shape[0]:
        ...
        hex_pattern = create_hex_pattern(hex_radius, hex_radius, hex_radius, avg_lab, palette_lab)
