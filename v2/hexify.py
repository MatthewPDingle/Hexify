"""
Hexify v2 CLI - Clean Architecture Entry Point

This module provides the command-line interface for the refactored Hexify.
It maintains the same interface as the original hexify.py while using
the cleanly architected v2 implementation.

Usage:
    python -m v2.hexify input_image.png
    python -m v2.hexify input_video.mp4 --colors 24 --processes 8
"""
...
from .config import MIN_PALETTE_COLORS
...
def create_output_directory(input_path: str) -> tuple:
    """
    Create output directories for processed files.

    Creates a directory structure:
        <input_name>/
            frames/     - Individual processed frames (for video)
            hexagons/   - Individual hexagon patterns

    Args:
        input_path: Path to the input file

    Returns:
        Tuple of (output_dir, frames_dir, hexagons_dir)
    """
...
def process_image(
    ...
    Args:
        ...
        num_palette_colors: Number of colors in palette
    ...
    processor = HexagonProcessor(
        ...
        hexagons_dir=hexagons_dir,
        chunk_size=chunk_size,
        save_hexagons=save_hexagons
    )

    # Get total hexagons for progress bar
    ...
    for i, color in enumerate(processor.palette.colors):
...
def main(
    ...
    """
    Main entry point for processing images or videos.
    ...
    Args:
        ...
        chunk_size: Hexagons per processing chunk
        save_hexagons: Whether to save individual hexagon images
        save_frames: Whether to save individual video frames
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate hexagonal pattern from input image or video (v2 clean architecture)."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Number of colors in the palette (default: 16)"
    )
    parser.add_argument(
        "-p", "--processes",
        type=int,
    ...
    if args.colors < MIN_PALETTE_COLORS:
        print(f"Minimum color palette size must be at least {MIN_PALETTE_COLORS}. Setting it to {MIN_PALETTE_COLORS}.")
        args.colors = MIN_PALETTE_COLORS
